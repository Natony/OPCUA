...
class PlcDataBuffer @Inject constructor(
    ...
    private val bufferScope = CoroutineScope(
        Dispatchers.Default + SupervisorJob() +
                CoroutineExceptionHandler { _, throwable ->
                    Log.e("PlcDataBuffer", "Coroutine error", throwable)
                }
    )
    ...
    private fun scheduleBatchEmit() {
        // Kiểm tra isActive trước khi emit
        if (!isActive || !pendingEmit.getAndSet(true)) {
            return
        }
        ...
        emitJob = bufferScope.launch {
            try {
                ...
                Log.e("PlcDataBuffer", "Error in batch emit", e)
            }
        }
    }
    /**
     ...
     */
    fun dispose() {
        Log.d("PlcDataBuffer", "Disposing PlcDataBuffer")
        isActive = false
        ...
        Log.d("PlcDataBuffer", "PlcDataBuffer disposed")
    }
